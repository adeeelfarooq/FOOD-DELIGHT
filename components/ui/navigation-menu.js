...
const NavigationMenuLink = React.forwardRef(({ className, ...props }, ref) => {
  return (
    <NavigationMenuPrimitive.Link
      ref={ref}
      className={cn(
        "relative flex w-full items-center justify-center rounded-md bg-background px-4 py-2 text-sm font-medium transition-colors hover:bg-muted hover:text-muted-foreground focus:outline-none focus:bg-accent focus:text-accent-foreground disabled:pointer-events-none disabled:opacity-50 data-[active]:bg-accent/50",
        className
      )}
      {...props}
    />
...
const NavigationMenuIndicator = React.forwardRef(({ className, ...props }, ref) => {
  return (
    <NavigationMenuPrimitive.Indicator
      ...
      {...props}
    >
      <div className="relative top-[55%] h-2 w-2 rotate-45 rounded-tl-sm bg-border shadow-md" />
    </NavigationMenuPrimitive.Indicator>
  );
})
